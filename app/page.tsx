...
import { Activity, Mic, MicOff, PhoneOff } from 'lucide-react';
...
// Voice chat component that connects to a specific NPC
  ...
  async () => {
    ...
    function VoiceChatInner({
      ...
      const appendSystemMessage = useCallback((text: string) => {
        setMessages((prev) => {
          const next = [...prev, { role: 'system' as Role, text }];
          return next;
        });
      }, []);
      ...
      const updateUserTranscript = useCallback(
        (event: AgentEvent) => {
          ...
          if (!turnId || counter === undefined) {
            ...
            upsertMessage({ role: 'user', turnId, text: content, chunks: [] }, { replace: true });
            return;
          }
          ...
          upsertMessage({ role: 'user', turnId, text: aggregatedText, chunks }, { replace: true });
        },
      ...
      // Pass NPC ID via metadata - this will be forwarded to the webhook
      const agent = useLayercodeAgent({
        ...
        onConnect: () => {
          ...
          appendSystemMessage(`Connected to ${npc.name}`);
        },
        onDisconnect: () => {
        ...
        <div className="space-y-3">
          <div className="flex gap-2">
            <button
              ...
              className="flex flex-1 items-center justify-center gap-2 rounded-md border border-red-700 bg-red-900/30 px-4 py-2 text-white transition hover:bg-red-900/50 disabled:opacity-50"
            >
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            >
              ...
              <span
                className={`ml-auto inline-flex h-2 w-2 rounded-full ${
                  userSpeaking ? 'animate-pulse bg-emerald-300' : 'bg-neutral-700'
                }`}
              />
            </div>
            <div
            ...
            >
              ...
              <span
                className={`ml-auto inline-flex h-2 w-2 rounded-full ${
                  agentSpeaking ? 'animate-pulse bg-emerald-300' : 'bg-neutral-700'
                }`}
              />
            </div>
          </div>
  ...
  async () => {
    ...
    function Game() {
      ...
      const handleNpcInteract = useCallback((npc: NPC) => {
        if (selectedNpc) return; // Already chatting
        setSelectedNpc(npc);
        setMessages([]);
        setError(null);
      }, [selectedNpc]);
      ...
      const handleMessage = useCallback((newMessages: Message[]) => {
        setMessages(newMessages);
        // Check for insufficient funds error
        const errorMsg = newMessages.find(m =>
          m.text.includes('insufficient_balance') || m.text.includes('402')
        );
        if (errorMsg) {
      ...
      const isConnected = selectedNpc !== null;
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <section className="rounded-xl border border-neutral-800 bg-black/30 p-5 flex flex-col">
                ...
                {/* Voice Controls - Only rendered when NPC is selected */}
                {selectedNpc && (
                  <div className="mb-4">
                    <VoiceChat
                      key={selectedNpc.id}
            ...
            <footer className="mt-6 text-center text-sm text-neutral-500">
              <p>
                Use <kbd className="px-2 py-1 bg-neutral-800 rounded text-neutral-300">WASD</kbd> or{' '}
                <kbd className="px-2 py-1 bg-neutral-800 rounded text-neutral-300">Arrow Keys</kbd> to move around the
                village.
              </p>
              <p className="mt-1">
                Press <kbd className="px-2 py-1 bg-neutral-800 rounded text-neutral-300">SPACE</kbd> near a villager to
